...
export default function BookingSeasonClient({
  id,
  isRenovation,
}: BookingSeasonClientProps) {
  ...
  const [shouldCollectClientContact] = useState<boolean>(
    shouldCollectCheckoutContact(accountInfo, clientContactObj),
  );
  ...
  const handleContinue = async () => {
    ...
    switch (bookingStep) {
      case "selection":
        ...
        if (
          !isRenovation ||
          (isRenovation && renovationType === "changeSeats")
        ) {
      ...
      case "payment":
        ...
        if (!isRenovation) {
          ...
          if (seasonBook.fulfilled.match(result)) {
            await dispatch(resetState());
            router.push(
              `/account/tickets/order/${result.payload.orderId}/success`,
            );
