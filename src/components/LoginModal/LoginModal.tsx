...
import { Alert, AlertColor, Backdrop, Box, Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, IconButton, Input, InputAdornment, Snackbar, TextField, Typography } from "@mui/material";
...
export default function LoginModal() {
    ...
    const [identifierCountryCode, setIdentifierCountryCode] = useState<string>(defaultAuthPhoneCountryCode);
    ...
    const handleEmailLogin = async () => {
        const result = await dispatch(login({ username: normalizeAuthIdentifier(identifier), password }));

        if (login.fulfilled.match(result)) {
            finishLogin(result.payload.emailVerified, result.payload.verificationStatus);
        }
    ...
    const handlePhoneLogin = async () => {
        ...
        try {
            ...
            setAlertSeverity("error");
            setAlertMessage(error instanceof Error ? error.message : "Error al iniciar sesión");
            setAlertOpen(true);
        } finally {
    ...
    const handleSendPhoneCode = async () => {
        ...
        try {
            ...
            setAlertSeverity("error");
            setAlertMessage(error instanceof Error ? error.message : "Error al enviar código SMS");
            setAlertOpen(true);
        } finally {
            setPhoneLoading(false);
        }
    ...
    const handlePasswordResetSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!email || !email.includes("@")) {
            setAlertSeverity("error");
            setAlertMessage("Ingresa tu correo electrónico para restablecer tu contraseña");
            setAlertOpen(true);
            return;
        }
        ...
        try {
            ...
            setAlertSeverity("error");
            setAlertMessage(error instanceof Error ? error.message : "Error al enviar correo de recuperación");
            setAlertOpen(true);
        } finally {
